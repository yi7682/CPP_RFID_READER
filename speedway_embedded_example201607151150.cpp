/*
 *****************************************************************************
	Boeing RFID Anti-collision Project
	Haifeng Niu
	July 2016
	Based on Impinj Application Example
 *****************************************************************************/
...
class CMyApplication
{
  ...
  public:
	...
	std::string targetEpc= "";
	//std::string targetEpc= "3034-3622-9C5F-4CC1-7790-012C";
    ...
    int
    configureAntenna (int id, int power);
    int 
	readTemp (void);
  ...
  char *                        pReaderHostName)
{
    ...
    exitFlag = false; 
    int cnt = 0;
    int txPower = 81;	// default value, 30dbm
    int temperatureThreshold = 60;   
    ...
    ifstream rConfig("readerConfig.txt");
	string rConfigLine;
	while(getline(rConfig, rConfigLine))
	{
		string indexS = rConfigLine.substr(0, 2);
		int index = str2int(indexS);
		switch (index)
		{
			case 1:
			{
				string txPowerS =  rConfigLine.substr(13, 2);
				txPower = str2int(txPowerS);
				cout << "TxPower index set to: " << txPower << endl;
				break;
			}
			case 2:
			{
				targetEpc = rConfigLine.substr(15, 29);
				cout << "Target EPC is: " << targetEpc << endl;
				break;
			}
			case 3:
			{
				string tempThrS =  rConfigLine.substr(26, 2);
				temperatureThreshold = str2int(tempThrS);
				cout << "Temperature threshold (C) set to: "
				     << temperatureThreshold << endl;
				break;
			}
		}
		//cout << rConfigLine << endl;		
	}
    
    // antenna id, power level 10+(x-1)/4
    if (configureAntenna(1, txPower) == 0)	
    {
		setGpo(4, 1);	//port, on/off
	}
	
	// test, print out the temperature
	readTemp();
    ...
    while (!exitFlag)   
    {
        if(readTemp() >= temperatureThreshold)
		{
			myApp.exitFlag = true;
			cout << "Temperature exceeds the threshold ("
		...
		if(targetFound == true && lightOn == false)
		{	
			setGpo(1, 1);
			lightOn = true;
			cnt = 0;
		}
		if(targetFound == false && lightOn == true)
		{
			cnt++;
			if (cnt >= 1)
			{
...
int
CMyApplication::str2int (const string &str) {
  stringstream ss(str);
  int num;
  ss >> num;
  return num;
...
CMyApplication::readTemp(void)
{
	...
	//CImpinjRequestedData* pImpinjRequestedData = new CImpinjRequestedData();
	//pImpinjRequestedData->setRequestedData(2004);	
	//pCmd->addCustom(pImpinjRequestedData);
    ...
    // add codes here to Check the LLRPStatus parameter.
    list<CParameter* >::iterator curr;	
    curr = pRsp->beginCustom();    
    for (
        curr = pRsp->beginCustom();
        curr != pRsp->endCustom();
        curr++)
    {
		if ((*curr)->m_pType == &CImpinjReaderTemperature::s_typeDescriptor)
		{
			CImpinjReaderTemperature* pReaderTemperature = (CImpinjReaderTemperature*) *curr;
			int temperature = pReaderTemperature->getTemperature();
			// print results
			cout<< "Current reader Temperature (C) is: "
				<< temperature << endl;
			return temperature;
		}
	}
...
int 
CMyApplication::setGpo(int port, int level)
{
	...
	// Set port to HIGH
	CGPOWriteData* pGpoWriteData = new CGPOWriteData();
	pGpoWriteData->setGPOPortNumber(port);
	pGpoWriteData->setGPOData(level);
	
	pCmd->addGPOWriteData(pGpoWriteData);
...
int
CMyApplication::configureAntenna (int id, int power)
{
    CSET_READER_CONFIG *        pCmd;
    CMessage *                  pRspMsg;
    CSET_READER_CONFIG_RESPONSE *pRsp;
    ...
    pAntennaConfiguration->setAntennaID(id);
    
    CRFReceiver * pCRFReceiver = new CRFReceiver();
    pCRFReceiver->setReceiverSensitivity(1);
    pAntennaConfiguration->setRFReceiver(pCRFReceiver);
    
    CRFTransmitter * pRFTransmitter = new CRFTransmitter();
    pRFTransmitter->setChannelIndex(1);
    pRFTransmitter->setHopTableID(1);
    pRFTransmitter->setTransmitPower(power);	// 10+(x-1)/4
    pAntennaConfiguration->setRFTransmitter(pRFTransmitter);   
         
    pCmd->addAntennaConfiguration(pAntennaConfiguration);
    ...
    if(m_Verbose)
    {
        printf("INFO: Antenna configuration set\n");
    }
